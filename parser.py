...
PER_SHARE_RE = re.compile(r'\b(per\s+(?:diluted|basic)\s+share|earnings\s+per\s+share|loss\s+per\s+share|per\s+share)\b', re.I)
    ...
    if not candidates:
        for rr in rows:
            ...
            if BASIC_RE.search(label) or DILUTED_RE.search(label):
                ...
                base = (2.0 if BASIC_RE.search(label) else 1.0) - (5.0 if ADJUSTED_RE.search(label) else 0.0)
                for ci, cell in enumerate(rr):
                    m = NUMBER_RE.search(cell)
                    if not m: continue
                    tok = maybe_merge_paren_token(m.group(0), rr, ci)
...
def extract_from_text(text: str) -> Optional[float]:
    ...
    for m in PER_SHARE_RE.finditer(text):
        ...
        low = window.lower()
        base = 0.0
        if ADJUSTED_RE.search(low): base -= 5.0
