...
def header_context_score(col_index: int, header_rows: List[List[str]]) -> float:
    if not header_rows:
        return 0.0
    score = 0.0
    # Look ONLY at this column’s header cells (top 2–3 header-like rows)
    cells = []
    for r in header_rows[:3]:
        if 0 <= col_index < len(r):
            cells.append(r[col_index].lower())
    cell_text = ' '.join(cells)
    # Strong preference for quarterly columns; de-prefer annual columns
    ...
    if ('twelve months ended' in cell_text or 'year ended' in cell_text
        ...
        score -= 3.0
    # Year parsed PER COLUMN (not max over neighbors)
    year_re = re.compile(r'\b(19\d{2}|20\d{2})\b')
    years = [int(m.group(0)) for m in year_re.finditer(cell_text)]
    if years:
        # Stronger boost for newer year, per column
        score += (max(years) - 1990) / 5.0
    # Bias LEFT (recent quarter is usually the leftmost data column)
    score -= 0.2 * col_index
    return score
...
def pick_best_quarter_column(header_rows: List[List[str]]) -> Optional[int]:
    if not header_rows:
        return None
    # Score each column by quarter/annual cues + recency
    ...
    for j in range(max_cols):
        ...
        if not t.strip():
            continue
        score = 0.0
        # Strong quarter preference; penalize annual
        ...
        if ('twelve months ended' in t or 'year ended' in t or '12 months' in t):
            score -= 8.0
        # Prefer newer year in this column
        years = [int(m.group(0)) for m in year_re.finditer(t)]
        if years:
            score += (max(years) - 1990) / 2.0
        # Resolve ties to the LEFT (recent quarter usually leftmost)
        score -= 0.15 * j
        if score > best[0]:
            best = (score, j)
    return best[1]
            ...
            for rr in band:
                ...
                for ci, cell in enumerate(rr):
                    ...
                    if best_col is None and header_rows:
                        # Build the header text for THIS column (use up to first 3 header-like rows)
                        hdr_bits = []
                        ...
                        # If this column looks like an annual column, skip it for EPS
...
def extract_from_text_eps_token(text: str) -> Optional[float]:
    ...
    for m in EPS_TOKEN_RE.finditer(text):
        ...
        # Scoring: prefer quarter context; penalize annual/6-month and adjusted.
        base = 0.0
        ...
        # Try number after “EPS” first, else anywhere in the window
...
def extract_from_text(text: str) -> Optional[float]:
    ...
    for m in PER_SHARE_RE.finditer(text):
        ...
        # Prefer the first number after; otherwise take the LAST number before (closest), and only then fall back to anywhere in the window.
        ...
        if not mnum:
            continue
        tok = mnum.group(0)
        # recompute loss/adjusted context over a bigger window that includes BEFORE text
