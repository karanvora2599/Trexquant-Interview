...
def extract_from_text_eps_token(text: str) -> Optional[float]:
    ...
    for m in EPS_TOKEN_RE.finditer(text):
        ...
        if not mnum:
            continue
        # Note: absolute position is approximated to window start; adequate for noise checks
        abs_s = start + mnum.start()
...
def _chunks(iterable, n):
    """Yield successive lists of length n from an iterable (streaming)."""
    it = iter(iterable)
    while True:
...
def _process_one_file(fp: Path) -> Tuple[str, str]:
    """Worker-safe file processor that returns (filename, EPS_str)."""
    try:
        eps = extract_eps_from_html(fp.read_bytes())
        return fp.name, '' if eps is None else _fmt_two_decimals_rounded(eps)
    except Exception:
        return fp.name, ''
...
def main(argv=None):
    ...
    ap.add_argument(
        '--batch-size', '-b',
        type=int, default=10,
        help='Number of files to process concurrently per batch (default: 10)'
    )
    ...
    with out_csv.open('w', newline='', encoding='utf-8') as f:
        ...
        for batch in _chunks(files, max(1, args.batch_size)):
            # Each batch is processed concurrently with up to batch_size threads
            with ThreadPoolExecutor(max_workers=max(1, args.batch_size)) as ex:
                for filename, eps in ex.map(_process_one_file, batch):
                    w.writerow([filename, eps])
