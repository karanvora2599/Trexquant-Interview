...
# NEW: "per share (diluted|basic) $X"
PER_SHARE_PAREN_CLASS_RE = re.compile(
    r'per\s+share\s*\(\s*(diluted|basic)\s*\)\s*\$?\(?\s*([+-]?\d+(?:\.\d+)?)\s*\)?',
    re.I
)
...
# Small helpers
# ======================
# TIGHTENED: 60-char radius so trailing "Adjusted ..." doesn't nuke GAAP EPS
def _near_adjusted(text: str, start: int, end: int, radius: int = 60) -> bool:
...
def header_context_score(col_index: int, header_rows: List[List[str]]) -> float:
    ...
    if QUARTER_RE.search(cell_text): score += 8.0
    if ANNUAL_RE.search(cell_text):  score -= 10.0
    if CONTINUING_RE.search(cell_text): score -= 5.0
    if GAAP_RE.search(cell_text): score += 2.0
...
def pick_best_quarter_column(header_rows: List[List[str]]) -> Optional[int]:
    ...
    for j in range(max_cols):
        ...
        if not t.strip():
            continue
        score = 0.0
        if QUARTER_RE.search(t): score += 12.0
        if ANNUAL_RE.search(t):  score -= 14.0
        if CONTINUING_RE.search(t): score -= 6.0
...
# ======================
# Extraction (tables) — same behavior
# ======================
    ...
    if eps_rows:
        for idx in eps_rows:
            ...
            for rr in band:
                ...
                if ADJUSTED_RE.search(label): base -= 12.0
                if BASIC_RE.search(label):    base += 6.0
...
def _skip_narrative_noise(ctx: str) -> bool:
    if ANNUAL_RE.search(ctx) and not QUARTER_RE.search(ctx): return True
    if CHANGE_RE.search(ctx): return True
    if PRIOR_RE.search(ctx):  return True
    if DISCONT_RE.search(ctx): return True
    return False
...
def extract_per_share_on_basis(text: str) -> Optional[float]:
    ...
    for m in BASIS_PER_SHARE_RE.finditer(text):
        tok = m.group(1)
        s, e = m.span(1)
        if _near_adjusted(text, s, e, radius=60):
            continue
...
def extract_narrative_net_sentence(text: str) -> Optional[float]:
    ...
    for m in NET_SENT_RE.finditer(text):
        ...
        tok  = m.group(2)
        start, end = m.span(2)
        if _near_adjusted(text, start, end, radius=60):
            continue
        ...
        if val is None or abs(val) > HARD_ABS_CAP or abs(val) < NEAR_ZERO:
            continue
        base = 1.5
        if QUARTER_RE.search(win): base += 1.0
        if GAAP_RE.search(win):    base += 0.6
...
def extract_from_narrative_strict(text: str) -> Optional[float]:
    ...
    for m in STRICT_EPS_PHRASE_RE.finditer(text):
        ...
        abs_s = m.end() + s_num
        abs_e = m.end() + e_num
        if _near_adjusted(text, abs_s, abs_e, radius=60):
            continue
        if _skip_narrative_noise(win.lower()):
            continue
        tok = mnum.group(0)
        ...
        if abs(val) >= 3.0 and not re.search(r'\bnet\s+(income|earnings|loss)\b', win, re.I):
            continue
        base = 0.5
        if 'dilut' in low: base += 0.75
        if 'basic' in low: base += 0.25
        base += _recency_score(win)
...
def extract_from_text_eps_token(text: str) -> Optional[float]:
    ...
    for m in EPS_TOKEN_RE.finditer(text):
        ...
        if _skip_narrative_noise(low):
            continue
        base = 0.0
        if re.search(r'\b(three\s+months|quarter(?:ly)?|q[1-4])\b', low): base += 2.0
        if re.search(r'\b(twelve\s+months|year(?:\s+ended)?|six\s+months)\b', low): base -= 2.0
...
def extract_from_text(text: str) -> Optional[float]:
    ...
    for m in PER_SHARE_RE.finditer(text):
        ...
        if QUARTER_RE.search(low):  base += 1.0
        if ADJUSTED_RE.search(low): base -= 2.0
...
def extract_per_share_paren_class(text: str) -> Optional[float]:
    """
    NEW catcher for 'per share (diluted|basic) $X'.
    Prefer diluted if both appear near; infer loss from nearby 'loss'.
    """
    best = None
    for m in PER_SHARE_PAREN_CLASS_RE.finditer(text):
        cls = (m.group(1) or '').lower()
        tok = m.group(2)
        s, e = m.span(2)
        if _near_adjusted(text, s, e, radius=60):
            continue
        ctx = text[max(0, s-240): min(len(text), e+160)]
        low = ctx.lower()
        if _skip_narrative_noise(low):
            continue
        if '.' not in tok:
        ...
        if val is None or abs(val) < NEAR_ZERO or abs(val) > HARD_ABS_CAP:
            continue
        # prefer diluted; basic only if no diluted candidate seen
        pref = 1 if cls == 'diluted' else 0
        score = pref + _recency_score(ctx)
        rv = (score, val)
        if best is None or rv[0] > best[0]:
            best = rv
    return None if best is None else best[1]
...
def extract_eps_from_html(html_bytes: bytes) -> Optional[float]:
    ...
    # 2) inline "Basic ... Diluted ..." catcher
    v = extract_basic_diluted_inline(text)
    if v is not None:
        return v

    # 2a) NEW: "per share (diluted|basic) $X"
    v = extract_per_share_paren_class(text)
    if v is not None:
        return v
