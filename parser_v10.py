...
# NEW: simple "… $0.16 per (basic|diluted) share" catcher
...
def extract_from_table(rows: List[List[str]]) -> Tuple[Optional[float], float]:
    ...
    if eps_rows:
        for idx in eps_rows:
            # penalize adjusted/Non-GAAP section headers just above
            ...
            for rr in band:
                ...
                # NEW: hard skip continuing operations rows for headline EPS
                ...
                for ci, cell in enumerate(rr):
                    ...
                    if not m: 
                        continue
                    # Skip annual columns unless we're forced later
                    ...
                    if best_col is not None and (ci not in (best_col, best_col + 1)):
                        continue
                    # NEW: column-level adjusted guard
            ...
            if row_cands:
                # Stronger tie-breaker: prefer higher header score, then |val|, then total score
    ...
    if not candidates:
        for rr in rows:
            ...
            base = 4.0
            if ADJUSTED_RE.search(label): base -= 12.0
            # hard skip continuing ops here too
            if CONTINUING_RE.search(label): 
                continue
            for ci, cell in enumerate(rr):
                m = NUMBER_RE.search(cell)
...
def extract_from_narrative_strict(text: str) -> Optional[float]:
    ...
    for m in STRICT_EPS_PHRASE_RE.finditer(text):
        win = text[m.end(): m.end() + 220]  # widened
        low = win.lower()
...
def extract_from_text_eps_token(text: str) -> Optional[float]:
    ...
    for m in EPS_TOKEN_RE.finditer(text):
        ...
        if not mnum: 
            continue
        s_num, e_num = mnum.span()
        if _near_adjusted(text, s_num, e_num, radius=80):
            continue
        tok = mnum.group(0)
        if '.' not in tok:
            continue
...
def extract_from_text(text: str) -> Optional[float]:
    ...
    for m in PER_SHARE_RE.finditer(text):
        ...
        if ANNUAL_RE.search(low):   base -= 2.0
        ...
        s_num, e_num = mnum.span()
        # reject adjusted near number
        abs_s = s_num  # window slice is from original text; using text directly below for guard
        abs_e = e_num
        if _near_adjusted(text, s_num, e_num, radius=80):
            continue
        ...
        val = normalize_num_token(tok, is_loss)
        if val is None or '.' not in tok or abs(val) > 20 or abs(val) < NEAR_ZERO:
            continue
        score = base + decimal_bonus(tok) + plausibility_penalty(val) + integer_penalty(tok, val)
        cands.append((score, val))
    if not cands:
...
def extract_simple_per_share(text: str) -> Optional[float]:
    """Catch: '$0.16 per (basic|diluted) share' with sign inferred from nearby 'net loss'."""
    best = None
    for m in SIMPLE_PER_SHARE_RE.finditer(text):
        ...
        if _near_adjusted(text, s, e, radius=90):
            continue
        # look back more than forward for "loss"
...
def extract_eps_from_html(html_bytes: bytes) -> Optional[float]:
    ...
    # NEW: simple per-share fallback before other generic passes
